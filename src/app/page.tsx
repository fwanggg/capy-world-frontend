...
export default function Home() {
  ...
  return (
    <div className="dark">
      {/* Top Nav */}
      <nav className="fixed top-0 w-full z-50 bg-[#10131a]/80 backdrop-blur-xl border-b border-outline-variant/10">
        <div className="flex items-center justify-between px-8 h-20 w-full max-w-7xl mx-auto">
          <div className="text-xl font-bold tracking-[0.2em] text-[#e1e2eb] font-headline uppercase">CAPYSAN</div>
          ...
          <div className="flex items-center gap-4">
            <button className="material-symbols-outlined text-[#94d3c3] hover:bg-[#1d2026] p-2 rounded-lg transition-all active:scale-95 duration-200">notifications</button>
            <button className="material-symbols-outlined text-[#94d3c3] hover:bg-[#1d2026] p-2 rounded-lg transition-all active:scale-95 duration-200">settings</button>
          </div>
        </div>
      </nav>
              ...
              <form onSubmit={handleAnalyze} className="max-w-xl mx-auto bg-surface-container-low p-1.5 rounded-full flex flex-col md:flex-row gap-2 shadow-2xl border border-outline-variant/10">
                <div className="flex-1 flex items-center px-6 py-3 gap-3">
                  ...
                  <input
                    ...
                    className="bg-transparent border-none focus:ring-0 focus:outline-none text-on-surface w-full placeholder:text-on-surface-variant/40 font-body text-sm"
                  />
                </div>
                <button
                  type="submit"
                  className="bg-primary-container text-on-primary-container font-bold px-8 py-3.5 rounded-full flex items-center justify-center gap-2 hover:brightness-110 transition-all group text-sm flex-shrink-0"
                >
                  Analyze Now
