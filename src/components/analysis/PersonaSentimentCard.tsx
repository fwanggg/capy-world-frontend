...
/** Stitch archetype labels: The Scrappy CTO, Enterprise Lead, or The {Profession} */
function getArchetypeLabel(profession: string | undefined, index: number): string {
  if (!profession) return 'Persona'
  const p = String(profession).toLowerCase()
  if (index === 0 && (p.includes('engineer') || p.includes('cto') || p.includes('developer'))) return 'The Scrappy CTO'
  if (index === 1 && (p.includes('lead') || p.includes('enterprise') || p.includes('manager'))) return 'Enterprise Lead'
  return `The ${profession.charAt(0).toUpperCase() + profession.slice(1)}`
}
...
/** Stitch: first row grid, second row border-t */
export function PersonaSentimentCard({ clones }: Readonly<PersonaSentimentCardProps>) {
  const [first, second] = clones.slice(0, 2)
  const renderRow = (clone: CloneResponse, i: number) => (
    <>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-on-surface">
              {getArchetypeLabel(clone.demographics.profession, i)}
            </p>
  ...
  return (
    <div className="p-8 bg-surface-container-lowest rounded-xl border border-outline-variant/10 shadow-inner">
      ...
      {first && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">{renderRow(first, 0)}</div>
      )}
      {second && (
        <div className="mt-8 pt-8 border-t border-outline-variant/10 grid grid-cols-1 md:grid-cols-4 gap-6">
          {renderRow(second, 1)}
        </div>
      )}
    </div>
