...
async function getLegacySecretKey(): Promise<import('jose').KeyLike> {
  const raw = SUPABASE_JWT_SECRET!.trim()
  if (raw.startsWith('{')) {
    try {
      const jwk = JSON.parse(raw)
      if (jwk.kty && jwk.alg) {
        return (await importJWK(jwk, jwk.alg)) as import('jose').KeyLike
      }
    } catch {
      /* fall through */
    }
...
export async function verifyJWT(token: string): Promise<JWTPayload | null> {
  try {
    ...
    const verified = await jwtVerify(cleanToken, key)
